...
| TD-2 | **LARGELY RESOLVED (M12)** — dead parallel dispatchers DELETED; kernel has a real execution surface (`kernel/execution.py::GoalExecutionRuntime`) delegating to the proven Operator, wired behind the opt-in `use_kernel_execution` flag (default off = legacy preserved). A **parity harness** (`test_m12_parity.py`) proves the kernel path is behaviorally equivalent to legacy for the same outcome — the objective gate for a future default flip. Remaining (behavior-changing, needs explicit go-ahead + real end-to-end validation): flip the default and delete the inline Operator branch. | `kernel/execution.py`, `bridge.py` | Ch 52/53 (mostly cleared) | 🟡 MOSTLY |
...
| TD-5 | **LARGELY RESOLVED** — `GoalExecutor._execute_step` no longer dispatches via an implicit if/elif ladder; it uses an explicit, inspectable capability→handler dispatch table (`_dispatch_table()` + `_dispatch_*` methods) and a data-driven `_DRY_RUN_BLOCKED` set. Behavior is provably unchanged (16-test characterization net in `test_executor_dispatch.py`, pinned green before AND after the refactor). This is the prerequisite for wiring handlers to the `CapabilityRegistry`. Remaining: point the table entries at registry-resolved capabilities (behavior-changing; deferred). | `executor.py` | Ch 16/22 (dispatch shape cleared) | 🟡 MOSTLY |
...
| TD-10 | **LARGELY RESOLVED** — the suite is now 1207 tests incl. M6–M12 gate/integration/property suites, plus dedicated failure-injection + adversarial + replay-determinism tests for the kernel execution path (`test_m12_failure_injection.py`: operator/factory/sink failures never crash the tick loop; malformed events ignored; goal lifecycle persisted + deterministically replayable from the event log). Remaining: broader end-to-end goal-completion benchmarks on real environments. | `tests/friday/*` | Ch 56 (mostly cleared) | 🟡 MOSTLY |
